...
OAuth2Client.prototype.request =
  function(opts, opt_callback, opt_dontForceRefresh, waitIteration) {
  ...
  this.transporter.request(opts, function(err, body, res) {
    ...
    else if(!hasAnyError || waitIteration > 4){
      opt_callback && opt_callback(err, body, res);
    }
    else {
      console.log('Drive error. StatusCode:', res.statusCode, ' Message:', body);
      setTimeout(function() {
        that.request(optsClones, opt_callback, opt_dontForceRefresh, waitIteration+1);
      }, Math.pow(2,waitIteration)+(Math.random()*1000));
    }
  });
};
